...
export default function handleGesture(canvas, state, event) {
	...
	if (!ctrlKey) {
		...
		console.table({
			// Canvas: { x: cW, y: cH },
			Before: { x: mX + state.x, y: mY + state.y },
			After: {
				x: (2 * offsetX / clientWidth - 1) / targetScale + state.x - mX * dS,
				y: (1 - 2 * offsetY / clientHeight) / targetScale + state.y + mY * dS
			},
			'': {}
		})
		// Write new state
	...
	console.table({
		state: { ...state, scale: normalizeScale(state.scale) },
		'': {x: '', y: '', scale: ''}
	})
}